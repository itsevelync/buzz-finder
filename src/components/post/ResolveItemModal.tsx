...
export default function ResolveItemModal({ itemId, itemName }: ResolveModalProps) {
    ...
    return (
        <>
            ...
            >
                <FaCheck className="text-xs" /> Mark as Found
            </button>
            ...
            {isOpen && (
                <div className="fixed inset-0 z-10000 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs">
                    <div className="bg-white rounded-lg shadow-xl border border-gray-100 max-w-md w-full p-6 animate-in fade-in zoom-in-95 duration-150">
                        ...
                        <div className="my-4 text-gray-600 space-y-2">
                            <p>
                                Did you find <strong className="text-foreground">{itemName}</strong>? You are changing its status to <strong className="text-foreground">&ldquo;Item Found&rdquo;</strong>.
                            </p>
                        </div>
                        ...
                        <div className="flex gap-3 justify-end pt-2">
                            <button
                                onClick={() => setIsOpen(false)}
                                className="px-4 py-2 font-medium border border-foreground/20 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-md transition"
                                disabled={isUpdating}
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                className="px-4 py-2 font-semibold text-white bg-buzz-blue hover:opacity-90 rounded-md transition shadow-xs flex items-center gap-1.5"
                                disabled={isUpdating}
                            >
